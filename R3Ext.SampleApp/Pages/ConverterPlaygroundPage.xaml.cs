...
public partial class ConverterPlaygroundPage : ContentPage
{
    ...
    private void SetupBindings()
    {
        ...
        _vm.WhenChanged(v => v.TemperatureCelsius)
            .Subscribe(_ => this.UpdateComfortSummary())
            .AddTo(ref _bindings);
    ...
    {
        double normalized = Math.Clamp((temperatureCelsius + 20.0) / 70.0, 0, 1);
        Color? cold = Color.FromRgb(0.0, 0.45, 0.9);
        Color? warm = Color.FromRgb(0.95, 0.4, 0.1);
    ...
    private static string DescribeComfort(double temperature, double humidity)
    {
        ...
        if (temperature < 28)
        {
            if (humidity < 40)
            {
                return "Warm and crisp — ideal weather.";
            }

            if (humidity < 70)
            {
                return "Pleasantly warm with mild humidity.";
            }
