...
public partial class DeepBindingPage : ContentPage
{
    ...
    private void SetupBindings()
    {
        ...
        _vm.WhenChanged(v => v.Deep.A.B.C.D.Leaf.Name)
            .Subscribe(name => this.UpdateValueLabel(DeepLeafNameLabel, ref _deepLeafNameUpdates, name))
            .AddTo(ref _bindings);
        ...
        _vm.WhenChanged(v => v.Mixed.NonNotify.Child)
            .Subscribe(child =>
            {
                ...
                this.UpdateValueLabel(MixedChildNameLabel, ref _mixedChildNameUpdates, child?.Name);
            })
            .AddTo(ref _bindings);
    ...
    {
        return value is null ? "(null)" : $"{value.GetType().Name} #{value.GetHashCode():X8}";
    }
                               ...
                               B = new SampleViewModel.DeepB
                                   {
                                       C = new SampleViewModel.DeepC
                                           {
                                               D = new SampleViewModel.DeepD { Leaf = new SampleViewModel.DeepLeaf { Name = $"Leaf {stamp}", }, },
                                           },
                                   },
                           },
                   };

        _vm.Mixed = new SampleViewModel.MixedRoot { NonNotify = new SampleViewModel.NonNotifyParent { Child = new Person { Name = $"Mixed {stamp}", }, }, };
    }
