...
The previous `ReactiveUICompatibleCommand` name was replaced by the shorter neutral `RxCommand`. Factory method shapes remain the same; just rename the type and static class usages.

### `WhenChanged` Binding Fallback
Bindings generated for chained property paths have a reflective fallback: if a generated entry is missing, `BindingRegistry` will attach `PropertyChanged` handlers to each owning object in the chain to rewire on intermediate replacements.
